...
def calc_sphere_mc():
    ...
    r0, k0, W0 = mc.initialize_sphere(nevents, ntrajectories, n_matrix_bulk, 
                                      n_sample, sphere_boundary_radius)
    ...
    (refl_indices, 
     ...
     transmittance_sphere)\
             = mc.calc_refl_trans_sphere(trajectories, n_matrix_bulk, n_sample, 
                                         sphere_boundary_radius, 
                                         p, mu_abs, mu_scat,  
                                         run_tir = False, 
                                         return_extra = True)
             
    return (refl_indices, trans_indices, k1_refl, k1_trans, norm_refl, 
            norm_trans, refl_per_traj, trans_per_traj, reflectance_sphere, 
            transmittance_sphere)
...
def test_mu_scat_abs_bulk():
    ...
    refl_per_traj, trans_per_traj,
    reflectance_sphere, 
    transmittance_sphere) = calc_sphere_mc()
    ...
    number_density = volume_fraction_bulk.magnitude/(4/3*np.pi*
                                        sphere_boundary_radius.magnitude**3)
    mu_abs_max = number_density*2*np.pi*sphere_boundary_radius.magnitude**2
    ...
    number_density = volume_fraction_bulk.magnitude/(4/3*np.pi*
                                        sphere_boundary_radius.magnitude**3)
    mu_scat_max = number_density*2*np.pi*sphere_boundary_radius.magnitude**2
